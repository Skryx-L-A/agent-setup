...
- **Auf peer gehören GUI-Tests auf Hyprland-Workspace 6 (2026-08-25, Vorgabe des Nutzers;
  schließt den bis dahin offenen Linux-Weg der Regel darüber).** Braucht eine Zusage eine ECHTE
  Sitzung — sichtbarer App-Start, Vollbild-Screenshot —, läuft sie ausschließlich dort; auf
  Workspace 6 ist alles erlaubt, weil er von Arbeit des Nutzers getrennt ist. Der HAUPTWEG bleibt
  trotzdem der verschachtelte oder headless Compositor (eigenes `XDG_RUNTIME_DIR`, `sway`
  headless oder ein anderer wlroots-Wirt); Workspace 6 ist die Ausnahme für das, was ohne echte
  Sitzung nicht geht. Danach bleibt kein Prozess stehen.
  **Eine layer-shell-Fläche schützt Workspace 6 NICHT (2026-08-25, gemessen):** wlr-layer-shell
  verankert eine Fläche an einem OUTPUT, nicht an einem Workspace. Gemessen in einem
  headless sway: nach `swaymsg workspace 6` steht dieselbe Overlay-Figur unverändert im Bild.
  Ein Overlay auf der echten Sitzung läge also über Arbeit des Nutzers, auf welchem Workspace er
  auch ist — solche Tests bleiben im verschachtelten Compositor, ohne Ausnahme.
...
- **Ein Test, der ein installiertes Werkzeug durch eine Attrappe ersetzt, bricht ZUERST den
  Symlink (2026-08-26, gemessen).** `werkzeuge_installieren` legt die Werkzeuge als `ln -sf` auf
  den Arbeitsbaum; ein `cat > "$FAKEHOME/.local/bin/<werkzeug>"` folgt dem Symlink und
  ueberschreibt die echte Datei im Repo. Am 26.08. stand `shell/wb-pane-write` danach als
  Attrappe im Arbeitsbaum, und der naechste Testlauf sah wie ein Fehler des geprueften Codes aus,
  obwohl er ein Fehler des Tests war. Also `rm -f` auf den Pfad, dann schreiben. Verwandt und aus
  derselben Nacht: eine **laufende** Skriptdatei wird nie in place ueberschrieben, sondern ueber
  `cp` auf eine neue Datei und `mv` darueber — das tauscht die Inode und laesst laufende Leser in
  Ruhe.

- **`trap cleanup INT TERM EXIT`, nicht nur `EXIT` (2026-08-26, gemessen).** Ein abgebrochener
  Testlauf liess seinen tmux-Testserver stehen, und der hielt die geerbte `flock` auf der
  Testlauf-Sperre weiter: `--liste` meldete „frei", `flock` wartete trotzdem. Der Halter einer
  `flock` ist die **offene Dateibeschreibung**, nicht der Prozess — deshalb ueberlebt die Sperre
  ihren Erzeuger, sobald ein Kind den Deskriptor geerbt hat, und `/proc/locks` nennt weiter eine
  PID, die `ps` nicht mehr kennt (`ino=$(stat -c %i <lockdatei>); grep " $ino " /proc/locks`).
  `fuser` reicht als Nachweis NICHT: es zeigt offene Deskriptoren, nicht Sperren.

- **Ein Werkzeug, das im Auftrag einer Wache handelt, wartet nie unbegrenzt auf einen fremden
  Prozess (2026-08-26, gemessen).** Ein `tmux send-keys` blieb 3 h 42 min ohne Antwort des tmux-Servers
  stehen und hat damit die gesamte Poll-Schleife der Kontextwache angehalten — keine Warnung,
  keine Notbremse, kein Logeintrag, fuenf Panes unbewacht. Jeder Aufruf an einen fremden Prozess
  bekommt eine Zeitschranke, und ein Abbruch wird protokolliert: ein stiller Abbruch ist derselbe
  Fehler in leise. Hergang:
  `20-projects/claude-workbench/sessions/session-2026-08-26-wache-stand-still.md`.
